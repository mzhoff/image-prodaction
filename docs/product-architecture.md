...
Этот документ фиксирует продуктовую архитектуру текущего production canvas в контексте будущей экосистемы Reverie.
...
Подробная целевая модель каталога Pipelines, публикации, endpoint policies, runs,
webhooks, usage dashboard и Library storage policy зафиксирована в
...
Library - workspace-level хранилище ассетов и reusable production objects.
...
Executable Pipelines и pipeline templates живут в отдельном разделе `Pipelines`.
Финальный результат pipeline может попасть в Library только согласно явно выбранной
storage policy. В этом случае Library asset хранит ссылку на pipeline, version и run.
...
- workspaces и memberships;
- documents и graph snapshots;
- asset metadata в PostgreSQL;
- originals и Library previews в S3-like storage;
- generation jobs и часть provider usage.

Frontend продолжает хранить transient UI state и технические промежуточные результаты,
которые пользователь не сохранял в Library.

Следующие backend-домены:
...
Сейчас OpenRouter вызывается через защищенный Next API layer, а глобальный key не
попадает в browser bundle. Generation jobs уже имеют серверный ledger, но provider
вызов еще выполняется внутри HTTP request.

Следующий этап отделяет provider contract, Workspace credential, queue и worker.
Frontend не должен знать детали provider integration. Вынос в отдельный deployable
AI service выполняется только при подтвержденной нагрузке.

API и worker contracts должны иметь:
...
Ближайшая согласованная архитектура AI execution использует зашифрованный provider
credential на уровне Workspace, provider-neutral contract и отдельный worker process.
Динамические feature flags и внутренние кредиты не входят в этот этап. Подробности:
