...
ChatModule `0.13.0`, не копируя их исходники. Универсальный пакет отвечает за
...
Локализация UI подключена через публичный `ChatLocalizationProvider` в
`src/shared/i18n/package-localization.tsx`. RU/EN-каталог принадлежит Production;
переводятся интерфейсные подписи и формат дат, содержимое сообщений и файлов
сохраняется. Отсутствие provider у другого потребителя сохраняет прежние
подписи ChatModule. Миграций БД для обновления 0.13.0 нет.

## Доступ к AI в закрытой бете

Перед показом composer продукт параллельно проверяет серверную конфигурацию
ассистента и сохранённое подключение текущего Workspace через существующий
membership-protected `GET /api/workspaces/:id/providers`. Проверка не вызывает
OpenRouter и не делает вывод о балансе по отсутствию ответа. Без подключения
пользователь видит инструкцию: владелец Workspace запрашивает ручную активацию у
администратора бета-теста в Telegram; после активации нажимает «Проверить доступ».
Невалидное подключение и неизвестное состояние показаны отдельно от отсутствия
активации. Серверные имена переменных и ключи в этом экране не отображаются.

Реальные права, лимит участника и credential проверяются заново на сервере при
каждом turn. Общий ключ Workspace используется генерациями и Ask AI. Ответ 402
сам по себе не доказывает, что именно лимит Workspace равен нулю: инструкция
просит администратора проверить и лимит, и общий резерв. Личный лимит участника
имеет отдельное сообщение с обращением к владельцу Workspace.

Продуктовые `AgentTurnError` используют `CHAT_*` — обязательный префикс публичного
контракта ChatModule. Без него SSE SDK заменяет сообщение на неизвестную ошибку.
Сквозной тест `workspace-provider-errors.test.ts` проверяет настоящий Next SSE
handler и опубликованный SDK, включая отсутствие автоматического повтора при
неактивированном бюджете. Изменений или форка пакетов ChatModule нет.
Общие ошибки связи отображаются через публичный `errorRenderer`: продукт меняет
только текст, сохраняя исходные trace, execution state и правила безопасного Retry.
...
- серверные defaults: 3 600 output tokens, 6 tool calls, 7 шагов и deadline
  75 секунд на turn; значения задаются конфигурацией хоста;
...
Пользователь может добавить до трёх файлов через выбор
файла, drag-and-drop или вставку из буфера, а затем отправить их вместе с
текстовым комментарием. Поддерживаются JPEG, PNG, WebP, HEIC/HEIF, UTF-8 документы
TXT/MD/CSV/JSON, видео MP4/MOV/WebM и аудио MP3/WAV/M4A/OGG/Opus/AAC/FLAC.
Лимит интерфейса — 8 МиБ на файл. Browser оптимизирует изображение до 2 048 px и 6 MB,
...
HEIC/HEIF преобразуется сервером в WebP; во время обработки видна карточка
с индикатором, затем миниатюра. Подробности и границы — в
[контракте чатов и референсов](production-chats-and-references.md).

Текстовые документы до 20 000 символов сервер передаёт модели как пользовательский
текст через продуктовый адаптер `composer-attachment-delivery.ts`. Проверяются
UTF-8, размер и контрольная сумма. PDF, DOCX и другие офисные контейнеры не включены:
для них нужен отдельный проверенный extractor/quarantine pipeline.

Видео и аудио проверяются существующими media processors и сохраняются как
managed files с предпросмотром. Текущая модель получает только имя и тип файла
с явным указанием, что содержимое не проанализировано. Расшифровка речи и анализ
видео не запускаются. Это продуктовая интеграция публичных extension points
ChatModule 0.12.1; пакеты не изменялись.

В режиме создания изображения медиа и документы нельзя отправить как image
references. На Home такие материалы переводят отправку в текстовый диалог;
в отдельном image-экране пользователь получает объяснение без потери материалов.
Сценарий создания Storyboard описан в [Home → Storyboard](./home-storyboard-creation.md).
...
## Материалы Create Image

Фото героя в пользовательском сообщении берётся из снимка `home_image_settings`,
закреплённого в `metadata.contextSelectors` конкретного сообщения. GET
`/api/chat/v1/home-image-settings` читает превью пакетами до 50 идентификаторов
после проверки владельца разговора и членства в Workspace. Ответ содержит только
имя героя и ID основного изображения, без паспорта и внутренних параметров.
До сохранения сообщения UI показывает выбранные материалы сразу, затем использует
снимок, подтверждённый сервером. Превью добавляются только при отображении:
они не меняют вложения ChatModule и не отправляют фото героя модели повторно.

Выбор референсов из Library — локальный черновик модального окна. Поиск и
пагинация сохраняют выбор; закрытие отменяет его. «Добавить» загружает оригиналы
выбранных изображений и передаёт их одним пакетом в композер. Ошибка загрузки
любого оригинала не прикрепляет частичный набор. Учитываются свободные места,
лимит модели и основные фото героев; для отдельного видеослота доступен один файл.
...
### Компактный композер в редакторах

Flow, Storyboard и Timeline используют компактный вариант `ProductionComposerFrame`,
тот же каркас, что Home. Загрузка, Library, выбор модели и отправка находятся в одной
строке; в Storyboard сюда же добавляется кнопка героев. Превью материалов находятся
в белой области без заголовков и отдельной полосы прокрутки. Жизненный цикл файлов,
сообщений и исполнения остаётся в ChatModule.

`AssistantModelSelector` меняет модель текущего диалога через runtime. Copilot Flow
принимает настроенную сервером модель и существующий список `PREFERRED_ANALYSIS_MODEL_IDS`,
как соавтор Storyboard и Timeline. Сервер проверяет список при каждом запросе;
проверки членства, доступа, бюджета и подтверждение действий сохраняются.
# Уточнение задачи одним нажатием

Canvas использует read-инструмент `assistant_ask_question`: один вопрос, исходное
намерение и ровно два коротких варианта. Карточка и отправка ответа находятся в
`src/shared/assistant`; кнопки рендерит штатный `quick-replies` из ChatModule.
Свободный ответ по-прежнему можно написать в композере.

Выбор отправляется следующим пользовательским сообщением в тот же разговор с
`selectedAction.payload.kind=assistant-answer`, ID вопроса, исходной задачей,
выбранным вариантом и его пояснением. Черновик и ещё не отправленные вложения
композера сохраняются. На время запроса варианты заблокированы; после ответа
в истории остаётся выбранный вариант. Обычный новый ответ закрывает старый вопрос.

В следующем ходе агент должен подготовить `pipeline_build`/`pipeline_update`,
используя существующий бриф и референсы. Повторный вызов вопроса возвращает
проверенный ответ из этого же разговора вместо новой формы. Сохраняются штатный
preview/подтверждение изменений графа и отдельное разрешение на платный анализ.
Для Extract описание сцены использует Composition, вход `image-0`, все выбранные
слои и согласованный формат текста. Ответ без вложений не теряет последний
прикреплённый референс.

Для уже сохранённых текстовых вопросов Extract «одним блоком / по разделам» UI
добавляет эти две кнопки без переписывания сообщений в БД. Совместимость ограничена
этим распознаваемым вопросом; новые варианты приходят структурированно от инструмента.
