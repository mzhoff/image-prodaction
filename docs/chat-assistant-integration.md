...
Привязка создаётся до первого сообщения и не заменяется второй вкладкой.
Уведомления «Изображение готово» и «Видео готово» сохраняются обычными
сообщениями ассистента, с исходным временем и действием «Открыть ноду».
Они находятся в общей истории; старые события документа добавляются туда
при открытии чата без повторов. Подробности: [`assistant-pet.md`](./assistant-pet.md).
