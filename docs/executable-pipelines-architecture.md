...
секций из Studio реализованы по плану
[Executable Pipeline Platform](./executable-pipeline-platform-plan.md).
Наличие раздела в этом документе не означает готовность соответствующей функции.
...
Первый интерфейс каталога показывает активную версию, типы и названия входов и
выходов, стабильный endpoint ID, количество вызовов, суммарные токены и среднюю
фактическую стоимость запуска. Метрики равны нулю, пока Runtime не создал ни
одного запуска.
...
В первой рабочей версии применяется endpoint-scoped Bearer token. Исходный token
показывается только один раз, а PostgreSQL хранит его SHA-256 hash. Выпуск ключей
пока выполняется операторской CLI-командой; пользовательский UI для управления
ключами относится к следующей итерации.
