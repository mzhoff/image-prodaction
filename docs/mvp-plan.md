...
> Исторический документ прототипа. Решения про локальное хранение ассетов и
> глобальный `OPENROUTER_API_KEY` больше не применяются: актуальный backend-контур,
> Workspace credentials, worker и usage описаны в
