...
Статус: backend MVP, строгая локальная email-авторизация, настройки аккаунта и CI/E2E реализованы; внешний email provider, staging и production-инфраструктура остаются отдельным release-этапом
...
4. Better Auth создаёт `user` и password account без активной session.
5. Idempotent onboarding service создаёт:
   - личный workspace;
   - membership с ролью `owner`;
   - при необходимости стартовый пустой document.
6. Mailpit локально (а позднее transactional email provider) получает письмо со ссылкой подтверждения.
7. Только после подтверждения адреса Better Auth создаёт HttpOnly session cookie.
8. Пользователь попадает в личный workspace; без подтверждения вход запрещён.

Политика управляется `AUTH_REQUIRE_EMAIL_VERIFICATION`. Безопасное значение по
умолчанию — `true`; `false` используется только как явный аварийный rollback.
...
Локальный статус: verification, reset, session management, SMTP-адаптер,
Mailpit, шаблоны писем, rate limits, security headers и request ID реализованы.
До production остаются Unisender Go, квоты, `generation_run` и полноценный
централизованный observability-контур.
...
Локальный/CI-статус: unit/integration, strict backend smoke, браузерный
critical-path Playwright, повторяемые миграции, production build, dependency
audit и Docker build реализованы. Staging, secret store, backup policy и
production monitoring намеренно отложены до появления инфраструктуры.
...
| Нет email provider | Нельзя отправлять письма реальным пользователям | Локально полный путь проверяется через Mailpit; Unisender Go подключается через готовую SMTP-границу до release |
...
- verification/reset проходят локальный E2E через Mailpit, а перед release — через production email;
