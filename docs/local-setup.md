...
### Hot reload внутри существующего Docker-контура

`compose.dev.yaml` запускает `next dev` и монтирует исходники. При пересоздании
`web` сохраняйте весь набор overlays действующего контейнера, включая
`compose.integration.yaml`, private-packages и локальный runtime overlay.
Не заменяйте его командой с одним `compose.yaml`.

Кэш работающего Next хранится в named volume `image-prodaction_web-dev-cache`,
а не в хостовой `.next`. Это отделяет серверные manifests от локальных сборок
и проверок типов: удаление или перезапись хостовой `.next` больше не ломает
активный Docker web. Кэш работающего сервера нельзя чистить на ходу.
Зависимости, добавленные после сборки базового образа, в dev-профиле подключены
точечными read-only mounts; полноценная сборка использует package-lock.json.

`worker` и `pipeline-worker` читают смонтированные исходники при запуске, но не
перезагружают их автоматически. После изменения обработчиков, особенно добавления
новых операций вроде `asset_ingest`, проверьте отсутствие активных заданий и
перезапустите соответствующий существующий контейнер. Иначе обновлённый web может
поставить задачу, которую старый процесс отклонит как
`unsupported_generation_operation`, даже при зелёном healthcheck. Перезапуск
`image-prodaction-worker-1` сохраняет его image, mounts, environment и networks;
пересоздавать web, базы или хранилище для этого не требуется.

При восстановлении 21 сентября пересоздан только `web` на прежнем образе, без
изменения runtime environment. Проверены readiness и авторизованное чтение
Runtime v2 из Content Hub; генерации не запускались. Базы и хранилище не менялись.
...
`AUTH_ALLOW_SIGN_UP=true` оставляет локальную регистрацию открытой. На закрытом
staging-контуре флаг равен `false`: `/register` возвращает пользователя на вход,
а backend отклоняет публичный sign-up. Одобренные staging-аккаунты создаются
интерактивной серверной командой, описанной в `docs/staging-deployment.md`.
