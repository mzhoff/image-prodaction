...
## 12. Visual LUT Builder and color look

Перед Scene Builder и video layer нужно добавить простой инструмент управления цветовым look.

Продуктовая цель:

- пользователь не изучает профессиональный color grading;
- пользователь выбирает или собирает визуальный look для серии изображений;
- один look можно применить к generated images, imported images и будущим video frames;
- интерфейс остается проще, чем профессиональные LUT-редакторы.

Минимальный состав:

- `Color Look` / `Visual LUT Builder` node;
- image input;
- LUT / look preset input optional;
- WebGL preview;
- `strength` 0-100%;
...
- `Color Warp`: паутинка / цветовая сетка с 6-12 опорными цветами, где пользователь двигает цвет в нужную сторону;
- `Color Contrast`: второй этап, где пользователь усиливает или смягчает светлоту / контраст выбранного цвета;
- нейтральные серые и кожа должны быть защищены дефолтными ограничениями, чтобы пользователь случайно не ломал изображение.

Технический принцип:

- не копировать профессиональный UI 3D LUT Creator;
- не выводить пользователю термины A/B, C/L, LAB, 3DL без необходимости;
- внутри можно использовать LAB / OKLab / OKLCH и генерировать 3D LUT;
- формат хранения / обмена выбрать отдельным решением: `.cube`, `.3dl` или другой формат;
- для браузерного preview использовать WebGL;
- для будущего video export использовать тот же look через серверный export pipeline.

Критерий готовности: пользователь может быстро собрать понятный color look, применить его к изображению и получить предсказуемый результат без знания LUT-форматов и профессиональной цветокоррекции.

## 13. Scene Builder and video
...
## 15. Предлагаемый порядок реализации
...
### Iteration 3.10: Visual LUT Builder / Color Look

- Добавить `Color Look` / `Visual LUT Builder` node.
- Поддержать image input и image output.
- Реализовать WebGL preview с `strength`.
- Добавить первый простой `Color Warp` UX: 6-12 цветовых селекторов вместо профессиональной сетки.
- Зафиксировать формат LUT для обмена отдельным решением: `.cube`, `.3dl` или другой.
- Проверить, что один look можно применить к серии изображений без ручной настройки каждого кадра.

Критерий готовности: пользователь может создать понятный цветовой look и применить его к серии изображений перед переходом к video layer.

### Iteration 3.11: Scene / video / 3D spikes
...
- Visual LUT Builder может быстро превратиться в сложный профессиональный color grading UI, поэтому MVP должен оставаться на уровне понятного color look, strength и простого color warp.
