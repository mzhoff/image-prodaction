...
- Результат генерации сохраняется как локальный image asset и показывается в preview-плейсхолдере самой ноды.
...
Почему так: пока нет backend storage и очередей, безопаснее держать лимиты на фронте и в API-route, чтобы случайно не отправлять в OpenRouter слишком тяжелые payloads.
