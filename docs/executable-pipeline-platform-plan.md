...
- legacy-определение входов и выходов по корням и листьям графа для старых
  draft, пока они не переведены в explicit contract mode;
- явные `Pipeline Input` / `Pipeline Output` и рекурсивный `Structured Output`
  для новых версионируемых контрактов;
