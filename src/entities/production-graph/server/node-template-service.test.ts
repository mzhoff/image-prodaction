...
import {
  ...
  saveNodeTemplate,
} from './node-template-service';

const workspaceId = '01900000-0000-7000-8000-000000000001';

test('template service scopes list and delete to the authenticated user and workspace', async () => {
  const repository = createMemoryRepository();
  const dependencies = createDependencies(repository);
  await saveNodeTemplate({
    data: { sourceText: 'Saved source', text: 'Private template', title: 'Renamed notes' },
    nodeType: 'textPrompt',
    userId: 'user-1',
    workspaceId,
  }, dependencies);

  const own = await listNodeTemplates({ userId: 'user-1', workspaceId }, dependencies);
  const other = await listNodeTemplates({ userId: 'user-2', workspaceId }, dependencies);
  assert.equal(own.length, 1);
  assert.equal(other.length, 0);
  assert.equal(own[0]?.snapshot.data.title, 'Renamed notes');

  await assert.rejects(
    deleteNodeTemplate({ templateId: own[0]!.id, userId: 'user-2', workspaceId }, dependencies),
    NodeTemplateNotFoundError,
  );
  assert.equal((await listNodeTemplates({ userId: 'user-1', workspaceId }, dependencies)).length, 1);
  await deleteNodeTemplate({
    templateId: own[0]!.id,
    userId: 'user-1',
    workspaceId,
  }, dependencies);
  assert.equal((await listNodeTemplates({ userId: 'user-1', workspaceId }, dependencies)).length, 0);
});

test('template service only keeps durable assets from the selected workspace', async () => {
  const repository = createMemoryRepository();
  const dependencies = createDependencies(repository, async (_userId, assetId) => ({
    status: 'ready',
    workspaceId: assetId === 'asset-allowed'
      ? workspaceId
  ...
  const data = result.template.snapshot.data as {
  ...
  assert.deepEqual(data.resultTexts, ['Saved output text']);
});
...
function createMemoryRepository(): NodeTemplateRepository {
  const records: NodeTemplateRecord[] = [];
  return {
    ...
    async upsert(input: SaveNodeTemplateRecord) {
      ...
      const record: NodeTemplateRecord = { ...input, createdAt: now, updatedAt: now };
