...
export const contextNodeHelp = {
  ...
  iterator: {
    ...
    availability: 'addable',
    capabilities: [
      'Бейджи подключённых image-референсов показывают название ноды-источника на голубом фоне по ширине текста; крестик при наведении отключает эту связь с поддержкой Undo.',
  ...
  subjectBuilder: {
    ...
    capabilities: [
      ...
      'В Studio бейджи над текстовыми полями переносят их содержимое в обычную Prompt или другое редактируемое поле; Alt копирует. Вычисляемый Result только копируется.',
      'Использует image refs и text notes, может подготовить AI-описание и canonical references.',
      'Publish Subject / Update Library явно сохраняет паспорт и до 24 референсов в общей серверной Library → Персонажи текущего Workspace.',
      'Паспорт можно создать и редактировать формой /library?section=subjects, затем загрузить в любой Subject Builder. Форма и нода используют одинаковые поля.',
      'Load загружает копию паспорта и изображений. Старые паспорта внутри канваса доступны с пометкой «в канвасе» и переносятся в общую библиотеку только явным сохранением.',
    ],
    execution: 'canvas-only',
    limitations: [
      ...
      'Для сохранения нужно имя до 120 символов; текстовое поле до 10000 символов, референсы до 24. Изменения в Library не обновляют канвасы автоматически.',
      'Конкурирующее сохранение устаревшей версии отклоняется без перезаписи. AI-генерация запускается отдельно в Subject Builder, не при сохранении или открытии формы.',
    ],
    portRules: [
