...
> {
  return {
    ...
    pasteImageAsset: (asset, position, targetNodeId, newNodeId) => {
      set((state) => {
        if (newNodeId && state.nodes.some((node) => node.id === newNodeId)) return {};
        ...
        const importNode = { ...node, id: newNodeId ?? node.id, data: { ...node.data, assetId: asset.id } } satisfies ProductionNode;
        return {
          ...withHistory(state),
          assets: nextAssets,
