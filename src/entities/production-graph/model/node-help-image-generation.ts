...
export const imageGenerationNodeHelp = {
  importImage: {
    ...
    capabilities: [
      'Загружает изображение или аудио в закрытое хранилище Workspace и отдаёт asset в граф.',
      'Определяет вид файла по содержимому: показывает изображение или аудиоплеер с длительностью, паузой и перемоткой.',
      'Может перенести image attachment из чата после подтверждения assistant proposal; аудио загружается через обычную Import.',
      'В явном executable pipeline сохранённый файл используется как проверенный asset.reference; в старом неявном режиме Import остаётся внешней входной границей.',
    ],
    execution: 'boundary',
    limitations: [
      'Не конвертирует файл, не генерирует результат и не распознаёт речь автоматически.',
      ...
      'PDF, текстовые документы, видео и произвольные файлы пока не поддержаны. Расширение само по себе не подтверждает формат.',
    ...
    summary: 'Добавляет изображение или аудиозапись, показывает подходящее превью и отдаёт файл в граф без обработки.',
  },
  imageToText: {
