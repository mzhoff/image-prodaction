...
import type { CompositionNodeData, ExportImageNodeData, ProductionNode, TextConcatNodeData } from '@/entities/production-graph/model/types';
...
export function expandDynamicInputPorts(node: ProductionNode, portId: string) {
  ...
  if (node.type === 'composition') {
    ...
    node.data = { ...data, layerInputCount: Math.max(2, currentCount, portIndex + 1) };
    return;
  }
  if (node.type === 'exportImage') {
    ...
    node.data = { ...data, imageInputCount: Math.max(1, currentCount, portIndex + 1) };
  }
}
...
export function expandTextConcatInputPorts(node: ProductionNode, portId: string) {
  ...
  node.data = { ...data, inputCount: Math.max(2, currentCount, portIndex + 1) };
}
