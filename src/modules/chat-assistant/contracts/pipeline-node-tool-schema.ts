...
const PIPELINE_SETTING_SCHEMAS = {
  ...
  language: { type: 'string', minLength: 1, maxLength: 80, description: 'Speech to text language hint or auto; Voice uses only the languages offered by its model.' },
  localText: { type: 'string', maxLength: 4_000 },
...
function createContractFieldsSchema(depth: number): Record<string, unknown> {
  const properties: Record<string, unknown> = {
    ...
    kind: { type: 'string', enum: ['text', 'number', 'boolean', 'image', 'audio', 'json'] },
