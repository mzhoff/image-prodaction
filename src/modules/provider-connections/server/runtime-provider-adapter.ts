import { withPaidCredential } from './paid-request-guard';
...
export function createRuntimeOpenRouterAdapter(): ProviderAdapter {
  if (process.env.AI_PROVIDER_RUNTIME !== 'fake') {
    ...
    return { ...adapter, execute: (request, context) => withPaidCredential(context.credential, () => adapter.execute(request, context)) };
  }
