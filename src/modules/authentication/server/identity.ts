...
export function identityPlugins() {
  ...
  if (!config) return [];
  if ([config.redirectURI, ...(config.additionalRedirectURIs ?? [])].some((uri) => new URL(uri).pathname !== '/api/auth/identity/callback')) throw new Error('Invalid Image Production Identity callback');
