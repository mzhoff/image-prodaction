...
export function ModelPreferencesSettings() {
  ...
  return <section className="model-preferences-settings" aria-labelledby="account-models-title">
    <h3 id="account-models-title">Избранные модели</h3>
    <p>Ваши модели для всех проектов и рабочих пространств. Добавляйте их в избранное и меняйте порядок стрелками — этот список появится во вкладке Favorite на Canvas.</p>
    <div className="model-selector-tabs" role="tablist" aria-label="Тип моделей">
      {MODEL_MODALITIES.map((type) => <button key={type} type="button" role="tab" id={`account-models-${type}`}
        ...
        }} onClick={() => { setModality(type); setQuery(''); }}>{LABELS[type]}</button>)}
    </div>
    <div id="account-models-panel" role="tabpanel" aria-labelledby={`account-models-${modality}`}>
      {(modality === 'video' && video.error) && <p role="alert">{video.error} <button type="button" onClick={video.reload}>Обновить каталог</button></p>}
      {(modality === 'image' && catalog.imageCatalogError) && <p role="alert">{catalog.imageCatalogError}</p>}
      <div className="model-preferences-columns">
        <div className="model-preferences-column">
          <h4>Все модели · A–Z</h4>
          <div className="model-selector-search"><input aria-label="Найти модель в настройках" placeholder="Найти модель" value={query} onChange={(event) => setQuery(event.target.value)} /></div>
          <ul aria-label="Все модели аккаунта">{available.map((item) => <li className="model-preferences-item" key={item.value}>
          ...
          {!available.length && <p>{query ? 'Модели не найдены.' : 'Нет доступных моделей.'}</p>}
        </div>
        <div className="model-preferences-column">
          <h4>Избранное · {preference.favorites.length}</h4>
          <ol aria-label="Порядок избранных моделей">{preference.favorites.map((id, index) => {
            const item = byId.get(id); const label = item?.label ?? id;
            return <li className="model-preferences-item" key={id}>
              <span className="dark-select-option-content">{item?.icon}<span title={item ? label : `${label} — сейчас недоступна`}>{label}{!item && ' · недоступна'}</span></span>
              <button type="button" className="model-preferences-move" aria-label={`Выше: ${label}`} disabled={disabled || index === 0} onClick={() => move(index, -1)}><ArrowUp size={14} /></button>
              <button type="button" className="model-preferences-move" aria-label={`Ниже: ${label}`} disabled={disabled || index === preference.favorites.length - 1} onClick={() => move(index, 1)}><ArrowDown size={14} /></button>
              {favoriteButton(id, label)}
            </li>;
          })}</ol>
          {!preference.favorites.length && <p>Нажмите звёздочку рядом с моделью. Новые модели добавляются в конец списка.</p>}
        </div>
      </div>
      <p className="model-preferences-status" role="status">{state.pending ? 'Сохраняем…' : state.loading ? 'Загружаем…' : state.ready && !state.error ? 'Изменения сохраняются автоматически.' : ''}</p>
      {state.error && <p role="alert">{state.error} <button type="button" onClick={() => void state.reload()}>Обновить настройки</button></p>}
      <p>Most popular показывает модели по числу ваших успешных запусков за всё время. Последняя открытая вкладка запоминается отдельно для каждого типа ноды. В конкретной ноде доступны только совместимые с ней модели.</p>
    </div>
  </section>;
}
