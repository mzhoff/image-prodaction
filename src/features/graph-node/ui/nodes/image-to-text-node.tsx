...
export function ImageToTextNode({ node, onStartConnection }: ImageToTextNodeProps) {
  ...
  const collapseLayout = getExtractNodeCollapseLayout(collapsed, {
    promptOpen: model.promptOpen,
    resultOpen: model.resultOpen,
  ...
  return (
    <>
      ...
      />
      {collapsed ? <span className="extract-node-collapsed-marker" aria-hidden="true" /> : null}
      {collapsed ? (
        <PortButton
          nodeId={node.id}
          portId="result"
          ...
          kind="text"
          label="Result"
          className="extract-node-collapsed-output-port"
          style={{ top: collapseLayout.portTop }}
