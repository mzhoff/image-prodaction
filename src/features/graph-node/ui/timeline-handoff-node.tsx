...
export function TimelineHandoffNode({ node, onStartConnection }: Props) {
  ...
  const ui_SENSITIVITY = useUiCatalog(SENSITIVITY, tUi);
  const ui_STAGES = useUiCatalog(STAGES, tUi);
  const ui_LANGUAGES = useUiCatalog(LANGUAGES, tUi);
    ...
    {isCollapsed ? <>{port('video', true)}{port('videoResult', true)}{port('frames', true)}{port('descriptions', true)}{port('timeline', true)}</> : <>
      <CollapsibleSection title="Video input" className="text-node-section timeline-input-section" sidePort={port('video')} dropTarget={{ nodeId: node.id, portId: 'video' }}>
        <div className="timeline-input-badge-row">
          {inputEdge ? <span className="timeline-input-badge" title={model.source?.name}>
            <span>{model.source?.name ?? tUi("Видео подключено")}</span>
            <button type="button" aria-label={tUi("Отключить видео")} data-node-interactive
              onPointerDown={(event) => event.stopPropagation()} onClick={() => deleteEdge(inputEdge.id)}><X size={12} /></button>
          </span> : <span className="timeline-input-placeholder">{tUi("Подключите видео")}</span>}
        </div>
      </CollapsibleSection>
      <div className="timeline-body" data-node-interactive onPointerDown={(event) => event.stopPropagation()}>
        <fieldset disabled={model.locked} className="timeline-settings timeline-analysis-settings">
          <SliderField label={tUi("Чувствительность")} thumbAriaLabel={tUi("Чувствительность смены сцен")} className="timeline-sensitivity"
            domain={{ min: 0, max: 100 }} step={1} value={sensitivityValue(model.data.threshold)}
            format={{ kind: 'semantic', mode: 'continuous', options: ui_SENSITIVITY }} ticks={ui_SENSITIVITY}
          ...
          <Button intent="neutral" appearance={analysis ? 'outline' : 'solid'} size="md"
            ...
            {analyzing ? tUi("Анализируем…") : analysis ? tUi("Анализировать снова") : tUi("Анализ")}
          </Button>
        </fieldset>
        {model.data.request ? <ProcessIndicator className="timeline-process" label={tUi("Обработка Timeline")}
          title={analyzing ? tUi("Анализ видео") : tUi("Создаём описания")}
          description={analyzing && analysisProgress ? ui_STAGES[analysisProgress.phase] : model.progress || tUi("Ожидаем начала обработки")}
          ...
          actions={<>
            {!model.busy ? <button type="button" onClick={() => void model.check()}>{tUi("Проверить результат")}</button> : null}
            {model.data.request.jobId ? <button type="button" onClick={() => void model.cancel()}>{tUi("Отменить")}</button> : null}
            {!model.busy && model.data.request.jobId ? <button type="button" onClick={() => void model.release()}>{tUi("Завершить запрос")}</button> : null}
          </>} /> : null}
        {model.data.message ? <p className="timeline-hint timeline-warning" role="alert">{model.data.message}</p> : null}
      </div>
      {analyzing ? <div className="timeline-analysis-skeleton" aria-label={tUi("Подготавливаем предпросмотр")} aria-busy="true">
      ...
      {!analyzing && analysis && shot && workspaceId ? <>
        <div className="timeline-body timeline-output-scope" data-node-interactive onPointerDown={(event) => event.stopPropagation()}>
          <SettingRow label={tUi("Выходы")} value={model.data.outputScope ?? 'selected'}
            options={[{ value: 'selected', label: tUi("Выбранный фрагмент") }, { value: 'all', label: tUi("Вся серия") }]}
            onChange={(value) => model.settings({ outputScope: value === 'all' ? 'all' : 'selected' })} wide />
        </div>
        <CollapsibleSection title={tUi("Фрагменты")} className="text-node-section timeline-fragments-section" sidePort={port('videoResult')}>
        ...
        <CollapsibleSection title={tUi("Описания")} className="text-node-section" sidePort={port('descriptions')}>
          <div className="timeline-review">
            <fieldset disabled={model.locked} className="timeline-settings">
              <SettingRow label={tUi("Пресет")} value="simple" options={[{ value: 'simple', label: tUi("Простой") }]} onChange={() => undefined} />
              <SettingRow label={tUi("Модель")} value={model.data.model} options={[...TIMELINE_MODEL_OPTIONS]} onChange={(value) => model.settings({ model: value })} wide />
              <SettingRow label={tUi("Язык")} value={model.data.language || 'system'} options={ui_LANGUAGES} onChange={(value) => model.settings({ language: value === 'system' ? undefined : value })} />
            </fieldset>
            <Button intent="neutral" appearance="soft" size="md" className="timeline-describe-current" disabled={model.locked}
              onClick={() => void model.start('describe', shot.id)}>{tUi("Описать этот фрагмент")}</Button>
            <PrimaryActionButton disabled={model.locked} onClick={() => void model.start('describe')}>
              {tUi("Создать все описания · ") + analysis.shots.length}
            </PrimaryActionButton>
        ...
        <CollapsibleSection title="Timeline" className="text-node-section timeline-document-section" sidePort={port('timeline')} defaultOpen={false}>
          <p className="timeline-hint timeline-body">{tUi("Вся серия: фрагменты, кадры и описания.")}</p>
        </CollapsibleSection>
        {model.outputs.busy ? <p className="timeline-hint timeline-body" role="status">{tUi("Подготавливаем видео и кадры для выходов…")}</p> : null}
        {model.outputs.error ? <div className="timeline-warning timeline-output-error" role="alert">{model.outputs.error}
          <Button appearance="outline" intent="neutral" size="sm" onClick={model.outputs.retry}>{tUi("Повторить")}</Button></div> : null}
      </> : null}
    </>}
