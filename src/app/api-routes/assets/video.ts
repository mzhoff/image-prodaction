...
export async function postAssetVideo(request: Request) {
  try {
    ...
    return await withStreamingUpload(request, MAX_VIDEO_BYTES, ['file', 'workspaceId', 'documentId', 'origin'], async ({ file, form }) => {
      ...
      const accepted = await submitAssetIngest({ ...fields.data, file, mediaKind: 'video', userId: session.user.id, signal: request.signal });
