...
import { summarizeBudgetWorkspaces } from '@/modules/provider-connections/server/platform/budget-summary';

const payload = z.object({ issuer: z.string().url(), subject: z.string().min(1).max(200) }).strict();
export async function handleBudgetSummary(request: Request, lookup = summarizeBudgetWorkspaces) {
  ...
  try {
    ...
    while (true) {
      ...
      if (size > 4096) { await reader.cancel(); return reply(413, 'REQUEST_TOO_LARGE'); }
      chunks.push(value);
    }
    const body = Buffer.concat(chunks).toString('utf8');
    if (!validPlatformSignature({
      body, secret, path: '/v1/platform/budget-summary',
    ...
    const workspaces = await lookup(issuer, parsed.data.subject, signal);
    return Response.json({ workspaces }, { headers: { 'Cache-Control': 'no-store' } });
  } catch { return reply(503, 'WORKSPACE_LOOKUP_UNAVAILABLE'); }
}
